...
class MainActivity : AppCompatActivity() {
    ...
    // Khai báo ClipboardManager ở đây
    private lateinit var clipboard: ClipboardManager // Sử dụng lateinit var để khởi tạo sau

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)

        // Lấy ClipboardManager và theo dõi thay đổi clipboard
        // Dòng này không gây lỗi "Val cannot be reassigned" vì 'clipboard' là lateinit var
        clipboard = getSystemService(Context.CLIPBOARD_SERVICE) as ClipboardManager
        clipboard.addPrimaryClipChangedListener {
            val clipText = clipboard.primaryClip?.getItemAt(0)?.text?.toString()
            if (!clipText.isNullOrBlank()) {
                ClipboardDataManager.addCopy(clipText)
                // Recreate activity để cập nhật UI
                recreate()
            }
        }
